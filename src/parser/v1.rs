...
use crate::{
    ...
    parser::{runtime_args::{parse_amount, parse_fee, parse_runtime_args, parse_transfer_args}, utils::timestamp_to_seconds_res},
    utils::{parse_account_hash, parse_public_key},
};

use auction::{parse_delegation, parse_redelegation, parse_undelegation};
use casper_types::{
    bytesrepr::Bytes, system::mint::{self, ARG_ID, ARG_SOURCE, ARG_TARGET, ARG_TO}, CLValue, Digest, InitiatorAddr, PricingMode, RuntimeArgs, TransactionArgs, TransactionEntryPoint, TransactionInvocationTarget, TransactionTarget, TransactionV1, TransactionV1Payload,
};
...
pub(crate) fn parse_v1_phase(v1: &TransactionV1, phase: TxnPhase) -> Vec<Element> {
    ...
    match meta.entry_point {
        ...
        _ => {
            let mut elements: Vec<Element> = v1_type(phase, &meta);
            match meta.target {
...
pub(crate) fn v1_type(phase: TxnPhase, item: &TransactionV1Meta) -> Vec<Element> {
